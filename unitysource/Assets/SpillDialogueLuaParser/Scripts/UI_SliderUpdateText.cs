...
public class UI_SliderUpdateText : MonoBehaviour
{
	...
	void Start()
	{
		sldr_Value.value = GLOBAL_old.Player.progress[DialogueStateKey].i;
	...
	void Update()
	{
		if (sldr_Value.value != GLOBAL_old.Player.progress[DialogueStateKey].i)
		{
			sldr_Value.value = GLOBAL_old.Player.progress[DialogueStateKey].i;
			txt_Value.text = sldr_Value.value.ToString();
		}
	...
	{
		GLOBAL_old.Player.progress[DialogueStateKey] = (int)sldr_Value.value;
		txt_Value.text = sldr_Value.value.ToString();
	}
}
